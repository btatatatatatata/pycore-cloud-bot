...
DISCORD_TOKEN = os.getenv('DISCORD_TOKEN') # Go To .env to edit the token!
BOT_NAME = 'PycoreCloud'
PREFIX = 'pc!'
YOUR_SERVER_IP = '127.0.0.1'
MAIN_ADMIN_ID = '1372237657207345183,1279500219154956419'  # Comma-separated admin IDs
VPS_USER_ROLE_ID = '1437440747530879086'
...
logger = logging.getLogger(f'{BOT_NAME.lower()}')
...
def add_field(embed, name, value, inline=False):
    ...
    return embed
def format_progress_bar(percent: float, length: int = 14) -> str:
    """Render a simple ASCII progress bar for embed displays."""
...
def get_host_disk_usage():
    """Return host disk usage as (percent, used_gb, total_gb)."""
    try:
    ...
    async def action_callback(self, interaction: discord.Interaction, action: str):
        ...
        if action == 'start':
            ...
            try:
                ...
                await execute_lxc(
                    f"""lxc exec {container_name} -- sh -c 'cat > /etc/motd << "EOF"
                Welcome To Pycore Cloud VPS!
...
async def health_dashboard(ctx):
    """Show a host health snapshot with progress-style bars."""
    cpu = get_cpu_usage()
    ram = get_ram_usage()
    disk_pct, used_gb, total_gb = get_host_disk_usage()
    embed = create_embed(f"{BOT_NAME} System Health", "Live host metrics (fake GUI view)", 0x00ccff)
    add_field(embed, "CPU", f"{format_progress_bar(cpu)}", True)
    add_field(embed, "RAM", f"{format_progress_bar(ram)}", True)
    add_field(embed, "Disk", f"{format_progress_bar(disk_pct)}\n{used_gb}GB / {total_gb}GB", True)
    add_field(embed, "Uptime", f"`{get_uptime()}`", False)
    add_field(embed, "Tips", "Keep deployments lean; clean unused containers and ports. Admins: monitor `!serverstats` regularly.", False)
    await ctx.send(embed=embed)
...
class ControlPanelView(discord.ui.View):
    """Interactive fake-UI control panel for quick user overview."""
    def __init__(self, ctx, target_user: discord.Member):
        super().__init__(timeout=240)
        self.ctx = ctx
        self.target_user = target_user
        self.target_user_id = str(target_user.id)
        self.current_section = "overview"
        self.embed = self.build_embed(self.current_section)
        self.refresh_buttons()
    def refresh_buttons(self):
        for child in self.children:
            if isinstance(child, discord.ui.Button):
                child.style = discord.ButtonStyle.success if child.custom_id == self.current_section else discord.ButtonStyle.secondary
    def build_embed(self, section: str):
        vps_list = vps_data.get(self.target_user_id, [])
        total_vps = len(vps_list)
        running = sum(1 for v in vps_list if v.get('status') == 'running' and not v.get('suspended', False))
        suspended = sum(1 for v in vps_list if v.get('suspended', False))
        whitelisted = sum(1 for v in vps_list if v.get('whitelisted', False))
        ports_alloc = get_user_allocation(self.target_user_id)
        ports_used = get_user_used_ports(self.target_user_id)
        ports_available = max(0, ports_alloc - ports_used)
        colors = {
            "overview": 0x00bcd4,
            "vps": 0x2ecc71,
            "ports": 0xe67e22,
        ...
        color = colors.get(section, 0x1a1a1a)
        embed = create_embed(f"{BOT_NAME} Control Panel", f"Interactive dashboard for {self.target_user.mention}", color)
        if section == "overview":
            cpu = get_cpu_usage()
            ram = get_ram_usage()
            disk_pct, used_gb, total_gb = get_host_disk_usage()
            add_field(embed, "Account", f"User: {self.target_user.mention}\nID: `{self.target_user.id}`", True)
            add_field(embed, "VPS Summary", f"Total: **{total_vps}** | Running: **{running}** | Suspended: **{suspended}** | Whitelisted: **{whitelisted}**", True)
            add_field(embed, "Ports", f"Allocated: **{ports_alloc}**\nUsed: **{ports_used}**\nAvailable: **{ports_available}**", True)
            gui_lines = [
                "+----------- CONTROL PANEL -----------+",
                f"| VPS Total : {total_vps:>2} | Running : {running:>2} | Susp : {suspended:>2} |",
                f"| Ports     : {ports_used:>2}/{ports_alloc:<2}  Free: {ports_available:<2}         |",
                f"| CPU  {format_progress_bar(cpu)} |",
                f"| RAM  {format_progress_bar(ram)} |",
                f"| Disk {format_progress_bar(disk_pct)} |",
                "+-------------------------------------+"
            ]
            add_field(embed, "Fake GUI", "\n".join(gui_lines), False)
            add_field(embed, "Host Snapshot", f"CPU: **{cpu:.1f}%**\nRAM: **{ram:.1f}%**\nDisk: **{used_gb}GB/{total_gb}GB ({disk_pct:.1f}%)**\nUptime: `{get_uptime()}`", False)
            add_field(embed, "Quick Actions", f"`{PREFIX}manage` • `{PREFIX}ports` • `{PREFIX}help` • `{PREFIX}health`", False)
        elif section == "vps":
            if not vps_list:
                add_field(embed, "No VPS", "This user has no VPS yet. Use `!create` (admin) to provision.", False)
            else:
                lines = []
                for i, vps in enumerate(vps_list, 1):
                    ...
                    if vps.get('whitelisted', False):
                        status += " (WHITELISTED)"
                    lines.append(f"#{i} `{vps['container_name']}` • {vps.get('config', 'Custom')} • {status}")
                add_field(embed, "VPS List", "\n".join(lines[:10]), False)
                if len(lines) > 10:
                    add_field(embed, "Note", f"Showing 10 of {len(lines)} VPS. Use `{PREFIX}list-all` for full list (admins).", False)
            add_field(embed, "Manage", f"`{PREFIX}manage` to open controls\n`{PREFIX}vps-stats <container>` for live metrics", False)
        elif section == "ports":
            forwards = get_user_forwards(self.target_user_id)
            add_field(embed, "Quota", f"Allocated: **{ports_alloc}** | Used: **{ports_used}** | Free: **{ports_available}**", False)
            if forwards:
                text = []
                for fwd in forwards[:8]:
                    created = datetime.fromisoformat(fwd['created_at']).strftime('%Y-%m-%d %H:%M')
                    text.append(f"ID {fwd['id']} • VPS `{fwd['vps_container']}` • {fwd['vps_port']} → {fwd['host_port']} • {created}")
                add_field(embed, "Active Forwards", "\n".join(text), False)
            else:
                add_field(embed, "Active Forwards", "No forwards created. Use `!ports add <vps_num> <port>`.", False)
            add_field(embed, "Port Helper", f"`{PREFIX}ports` to view commands\n`{PREFIX}ports add` to create forward\n`{PREFIX}ports remove <id>` to delete", False)
        else:  # support
            add_field(embed, "Support Links", f"• `{PREFIX}help` for all commands\n• `{PREFIX}panel` to reopen this UI\n• `{PREFIX}info @user` (admin) for account audit", False)
            add_field(embed, "Good Practices", "• Keep snapshots before risky changes\n• Clean unused ports and VPS\n• Back up data regularly", False)
            add_field(embed, "Need Assistance?", "Open a ticket with timestamps and container names for faster help.", False)
        return embed
    async def switch(self, interaction: discord.Interaction, section: str):
        if str(interaction.user.id) != str(self.ctx.author.id):
            await interaction.response.send_message(embed=create_error_embed("Access Denied", "Only the command author can use this panel."), ephemeral=True)
            return
        self.current_section = section
        self.embed = self.build_embed(section)
        self.refresh_buttons()
        await interaction.response.edit_message(embed=self.embed, view=self)
    @discord.ui.button(label="Overview", style=discord.ButtonStyle.success, custom_id="overview")
    async def overview_button(self, interaction: discord.Interaction, button: discord.ui.Button):
        await self.switch(interaction, "overview")
    @discord.ui.button(label="VPS", style=discord.ButtonStyle.secondary, custom_id="vps")
    async def vps_button(self, interaction: discord.Interaction, button: discord.ui.Button):
        await self.switch(interaction, "vps")
    @discord.ui.button(label="Ports", style=discord.ButtonStyle.secondary, custom_id="ports")
    async def ports_button(self, interaction: discord.Interaction, button: discord.ui.Button):
        await self.switch(interaction, "ports")
    @discord.ui.button(label="Support", style=discord.ButtonStyle.secondary, custom_id="support")
    async def support_button(self, interaction: discord.Interaction, button: discord.ui.Button):
        await self.switch(interaction, "support")
@bot.command(name='panel')
async def control_panel(ctx, user: discord.Member = None):
    """Open an interactive control panel (admins can inspect another user)."""
    target_user = ctx.author
    if user:
        user_id_check = str(ctx.author.id)
        if is_main_admin_user(user_id_check) or user_id_check in admin_data.get("admins", []):
            target_user = user
        else:
            await ctx.send(embed=create_error_embed("Access Denied", "Only admins can open the panel for another user."))
            return
    view = ControlPanelView(ctx, target_user)
...
class HelpView(discord.ui.View):
    def __init__(self, ctx):
        ...
        self.command_categories = {
            ...
            "name": "👤 User Commands",
            "commands": [
                (f"{PREFIX}panel [@user]", "Open interactive control panel (Admin can inspect others)"),
                (f"{PREFIX}ping", "Check bot latency"),
                (f"{PREFIX}uptime", "Show host uptime"),
                (f"{PREFIX}health", "Host health snapshot"),
            ...
            "commands": [
                    (f"{PREFIX}health", "Host health snapshot (progress bars)"),
...
if __name__ == "__main__":
    if DISCORD_TOKEN:
        ...
        logger.warn("The Bot Crashed With Critical ERROR:")
        time.sleep(0.1)
        logger.error("No Discord token found in .env file! Please Set It!")
        time.sleep(0.3)
        sys.exit(1)
